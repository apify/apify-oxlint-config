# Changelog

## 0.2.8 - 2026-05-07

### Bug Fixes
- Disable vitest/require-to-throw-message (mirrors the jest variant) (#12)


## 0.2.7 - 2026-05-07

### Bug Fixes
- Own the OxlintConfig type alias so consumers don't trip TS2883 (#11)


## 0.2.6 - 2026-05-07

### Bug Fixes
- Turn off Jest plugin rules already covered by the Vitest plugin (#10)


## 0.2.5 - 2026-04-28

### Bug Fixes
- Silence noisy vitest rules (valid-expect, valid-describe-callback) (#9)


## 0.2.4 - 2026-04-27

### Bug Fixes
- Drop import-js/no-extraneous-dependencies until oxlint supports it natively (#8)
- Turn off noisy/buggy vitest rules that the plugin auto-enables (#6)


## 0.2.3 - 2026-04-27

### Features
- Add vitest plugin alongside jest (#5)


## 0.2.2 - 2026-04-24

### Features
- Add defineConfig helper that pre-merges the preset (#4)


## 0.2.1 - 2026-04-24

### Chores
- Ship TypeScript declarations for the shared config (#3)


## 0.2.0 - 2026-04-24

### Features
- Redefine config in JS, drop index.json (#2)


## 0.0.1 - 2026-04-23

Initial release.
### Bug Fixes
- Broaden config-file glob to match multi-part variants

### Chores
- Initial commit
- Drop test framework globals
- Drop react and react-hooks from base plugins



